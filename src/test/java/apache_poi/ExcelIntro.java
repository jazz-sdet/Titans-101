...
@Test
    ...
    File excelFile=new File("src/test/resources/TestExcelFine.xlsx");
...
XSSFSheet sheet = FileUtils.openExcelSheet("TestExcelFine", "Sheet1");

        Row row =sheet.getRow(0);

        int totalRows=sheet.getPhysicalNumberOfRows();

        for (int i=0; i<totalRows; i++){//iterate rows
            Row currentRow=sheet.getRow(i);
            for(int j=currentRow.getFirstCellNum(); j<currentRow.getLastCellNum(); j++){ //iterate cells
